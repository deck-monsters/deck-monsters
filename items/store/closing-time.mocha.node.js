...
describe.only('./items/store/closing-time.js', () => {
	it('tells you closes in 8 hours', () => {
		const now = new Date();
		const closingTime = getClosingTime(now.setHours(now.getHours() + 8));
	...
	it('tells you closes in 4 hours', () => {
		const now = new Date();
		const closingTime = getClosingTime(now.setHours(now.getHours() + 4));
