...
- `look at monsters in detail` — View your monsters with full stats
- `look at [monster]` — View a specific monster's stats (e.g. `look at Fluffy`)

### The Ring

- `send [monster] to the ring` — Send a monster to the ring (e.g. `send Fluffy to the ring`)
- `send monster to the ring` — Select a monster and send to the ring
- `call [monster] out of the ring` — Call a monster out of the ring (e.g. `call Fluffy out of the ring`)
- `summon a boss` — Call a boss into the ring to fight your monster (3 per day)
- `look at the ring` — See which monsters are currently fighting

### Cards

- `look at cards` — View your cards
- `look at card inventory` — View equipped and unequipped cards together
- `look at all cards` — Alias for card inventory
- `look at inventory` — View all cards and items across your character and monsters
- `look at [card name]` — View details about a specific card (e.g. `look at Hit`)
- `look at card [card name]` — View details about a specific card (e.g. `look at card Heal`)
- `look at deck` — View your full card inventory
- `unequip [card] from [monster]` — Remove a card from a monster back to your cards (e.g. `unequip Hit from Fluffy`)
- `unequip [count] [card] from [monster]` — Remove multiple copies of a card from a monster (e.g. `unequip 2 Hit from Fluffy`)
- `unequip all from [monster]` — Clear a monster's full deck back to your inventory (e.g. `unequip all from Fluffy`)
- `move [card] from [monster A] to [monster B]` — Move a card directly between monsters (e.g. `move Hit from Fluffy to Fang`)
...
- `visit the shop` — Browse and buy items from the merchant
- `sell to the shop` — Sell cards or items to the merchant
...
- `look at player handbook` — Read the full player handbook
- `look at monster manual` — Browse all monster types
- `look at dm guide` — Read the dungeon master guide
