...
export interface LeaderboardPanelProps { roomId?: string; initialScope?: Scope; headerActions?: ReactNode }

function LeaderboardTableRegion({ label, children }: { label: string; children: ReactNode }) {
  return <div className="leaderboard-table-region" role="region" aria-label={label} tabIndex={0}>{children}</div>;
}
  ...
  return (
    ...
    <section className="surface-panel leaderboard-panel">
        ...
        {kind === 'players' && scope === 'room' && roomId && (
          ...
          </table></LeaderboardTableRegion>
        )}

        {kind === 'monsters' && scope === 'room' && roomId && (
          <LeaderboardTableRegion label="Scrollable room monster rankings"><table aria-label="Room monster rankings" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)', textAlign: 'left' }}>
                <th style={{ padding: '0.35rem' }}>#</th>
                <th style={{ padding: '0.35rem' }}>Name</th>
        ...
        {kind === 'players' && scope === 'global' && (
          <LeaderboardTableRegion label="Scrollable global player rankings"><table aria-label="Global player rankings" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)', textAlign: 'left' }}>
                <th style={{ padding: '0.35rem' }}>#</th>
                <th style={{ padding: '0.35rem' }}>Name</th>
        ...
        {kind === 'monsters' && scope === 'global' && (
          <LeaderboardTableRegion label="Scrollable global monster rankings"><table aria-label="Global monster rankings" style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border)', textAlign: 'left' }}>
                <th style={{ padding: '0.35rem' }}>#</th>
                <th style={{ padding: '0.35rem' }}>Name</th>
