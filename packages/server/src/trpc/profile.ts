			...
			for (const { roomId } of memberRooms) {
				try {
					await roomManager.runSerializedEngineWork(roomId, async () => {
						...
						// "Deliberately narrow: a player who renamed themselves in-game keeps
						// that name." This mirrors Game.getCharacter's name-healing constraint.
						if (character?.givenName === previousDisplayName) {
