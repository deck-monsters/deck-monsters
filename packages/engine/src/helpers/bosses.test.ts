import { expect } from 'chai';

import { randomContestant } from './bosses.js';
import HitCard from '../cards/hit.js';
import FleeCard from '../cards/flee.js';

describe('./helpers/bosses.ts', () => {
	describe('randomContestant', () => {
		it('can generate a contestant', () => {
			const contestant = randomContestant();

			expect(contestant).to.have.all.keys(
				'monster',
				'character',
				'channel',
				'channelName',
				'isBoss'
			);
		});

		it('generates monsters which cannot flee', () => {
			const { monster } = randomContestant();

			expect(monster.canHoldCard(HitCard)).to.equal(true);
			expect(monster.canHoldCard(FleeCard)).to.equal(false);
		});

		it('generates characters which cannot flee', () => {
			const { character } = randomContestant();

			expect(character.canHoldCard(HitCard)).to.equal(true);
			expect(character.canHoldCard(FleeCard)).to.equal(false);
		});

		it('generates characters with only cards a boss can hold', () => {
			for (let i = 0; i < 10; i++) {
				const { character } = randomContestant();

				expect(character.deck.every((card: { constructor: { noBosses?: boolean } }) => !card.constructor.noBosses)).to.equal(true);
			}
		});
	});
});
