...
const getWinnings = (matches: number, cost: number): number =>
	Math.round(Math.pow(10, 0.6 * matches) * cost);

export class LotteryTicket extends BaseScroll {
	...
	action({ channel, channelName, character }: { channel: any; channelName?: string; character: any }): Promise<boolean> {
		return Promise.resolve()
			.then(() => {
				const characterNumbers = getTicketNumbers();
				const winningNumbers = getTicketNumbers();

				const matches = characterNumbers.reduce(
					(numberOfMatches, number, currentIndex) =>
						winningNumbers[currentIndex] === number ? numberOfMatches + 1 : numberOfMatches,
					0
				);
				...
				return Promise.resolve(channel)
					.then(({ channelManager }: any = {}) => channelManager && channelManager.sendMessages())
					.then(() => ({ winningNumbers, matches }));
			})
			.then(({ winningNumbers, matches }: { winningNumbers: number[]; matches: number }) => {
				if (matches > 0) {
