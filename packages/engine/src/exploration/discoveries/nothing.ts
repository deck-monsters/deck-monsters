...
export class NothingCard extends BaseDiscoveryCard {
	...
	get stats(): string {
		...
		const { text: flavor } = getFlavor('nothing' as any, (this.constructor as any).flavors);
...
(NothingCard as any).cardType = 'Nothing';
(NothingCard as any).probability = 80;
(NothingCard as any).description = 'Your monster finds...';

(NothingCard as any).flavors = {
	nothing: [
