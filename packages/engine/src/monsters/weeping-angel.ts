...
const NATIONALITIES = ['English', 'Welsh', 'Scottish'];

const DESCRIPTORS = ['frutier', 'nuttier'];
...
class WeepingAngel extends BaseMonster {
	...
	get nationality(): string {
		return this.options.nationality as string;
	}
	...
	get description(): string {
		const { pronouns } = this;
		const article = this.color[0].match(/[aeiou]/i) ? 'An' : 'A';
		return `${article} ${this.color} weeping angel. On meeting ${pronouns.him} one might form the following three impressions: that ${pronouns.he} was ${this.nationality}, that ${pronouns.he} was intelligent, and that ${pronouns.he} was ${this.descriptor} than a treeful of monkeys on nitrous oxide.`;
	}
}

WeepingAngel.creatureType = WEEPING_ANGEL;
WeepingAngel.class = CLERIC;
(WeepingAngel as any).acVariance = 1;
(WeepingAngel as any).hpVariance = 1;
(WeepingAngel as any).description = `
