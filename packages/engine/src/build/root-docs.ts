...
// Kept as plain sentence text (not the blockquote markup below) so a grep for the exact
// wording — this file's own tests, or anyone else's — keeps matching regardless of how
// the root files format it.
export const GENERATED_DOC_NOTICE = [
	'Generated from packages/engine/src/build — do not edit this file directly.',
	'Run pnpm run build:docs.',
].join('\n');

/** `GENERATED_DOC_NOTICE` as the single blockquote line the root files render: two
 * sentences on their own lines read as a run-together paragraph on GitHub (no visual cue
 * that it's a notice rather than doc prose), so this joins them onto one `> ` line and
 * marks the command as code. */
const GENERATED_DOC_NOTICE_BLOCKQUOTE = `> ${GENERATED_DOC_NOTICE.split('\n').join(' ').replace(
	'pnpm run build:docs',
	'`pnpm run build:docs`'
)}`;
...
const generatedMarkdown = (title: string, body: string): string =>
	normalizeLineEndings(
		normalizeMarkdownSpacing(
			`# ${title}\n\n${GENERATED_DOC_NOTICE_BLOCKQUOTE}\n\n${body.replace(/^\n+/, '')}`
		)
	);
