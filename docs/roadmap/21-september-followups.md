...
| A | One vocabulary for the world (`train`, `call out`, `dismiss`, pronouns…) + `docs/voice-and-wording.md` | Tier 3 doc, Tier 2 apply | ✅ done (`3864145` voice doc; `8896b6d`, `1372f8f`, `f244a3b`, `ae39f1a` apply, #163; review found 4 Important — leaked `battle`/`encounter`/`arena`, `Owner`/`L3`, `in`-operator pronoun-key check accepting `toString`, #163 silent on the never-working Discord `/spawn [type] [name]` — fixed in `e472c19`, `712b317`) | — |
...
| G | Implement [17 — Pixel Art Fight Animations](../archive/roadmap/17-pixel-art-fight-animations.md) | Tier 3 design, Tier 2 implement | 🔧 G1 ✅ (`a2d8e7a`, review fix `8235137`); G2 landed (`64c3e5a`…`5368228`), live-verified in Test Room A, review-fix round landed (`2368f79`…`8eed8c3`: time-driven settle, single theme store, roster churn, payload validation, canvas sizing, sprite legibility); orchestrator follow-ups `3a29ae0` (poses derived from each literal map — the redraw had shipped six identical frames), `7688719` (settle timer re-arms on an early wake; bug #162), `6912e7d` (hold fighters through the fade). Live re-verified: band fades ~2 s after `Fight concluded`, fallen pose visible | `a2d8e7a`, `b18fc31`, `8235137`, `64c3e5a`, `265807b`, `4e40ef7` |
| H | Reorganise the roadmap: archive what shipped, make remaining work obvious | Tier 1 survey → Tier 2 apply | ✅ done (`16395c3` archive 13 shipped plans under `docs/archive/roadmap/` + `22-small-leftovers.md`; `0630a8d` README as Now/Next/Deferred/Shipped; `b05dfed` repointed links; 140 relative links checked, 0 broken) | — |
| I | Encode what worked in this process into `AGENTS.md` / `docs/agents/subagents.md` | orchestrator | ✅ done (Standing Instruction 6 \"plan in the roadmap\"; shared-worktree rules, review shape, fix-round resume, live re-check, provider fallback, real-object tests, recording checks in `docs/agents/subagents.md`; `game-primer.md` gained derived monster slots, `respawnAt`, combat DTO + pixel layer; `local-testing-guidelines.md` gained the x11grab/window recipe) | — |
