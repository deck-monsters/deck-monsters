...
| [10 — Bug Fixes](10-bug-fixes.md) | Open bugs, UX polish, cleanup | 🔧 Active — 3 non-blocking items from the Sept 2026 live-play pass, plus 11 triaged mobile UI/copy bugs (#98–#108) |
...
Real-time sync bugs, quick actions, batch-equip UX, card shop room-scoping, DMG/CARDS content differentiation (#3), the 2026-08-03 audit fixes (#51–#85), and Fastify tRPC batch `maxParamLength` 404s (#86) are all archived in `10b-bugs-fixed.md`. The preset casing and parsing fixes (#87–#88) are archived there too, along with the September 2026 live-play fixes (#89–#97). `10-bug-fixes.md` now carries three open non-blocking items from that pass: the email-defaulted profile name migration (follow-up to #95), cards that emit two roll blocks in the same tick, and the burst of messages at the very start of a fight. It also carries a **September 16 2026 mobile UI triage** (#98–#101, none fixed yet): feed text clipped off the right edge of both panes because `.event-feed`'s padding sits on the Virtuoso scroller whose absolutely-positioned viewport resolves `width:100%` against the padding box (#98); the fight-log event trace rendering raw engine markup (#99); unpluralized `in 1 rounds` (#100); and tofu glyphs in the turn banner, from the U+2680–U+2685 dice divider JetBrains Mono does not ship (#101). A second group covers what the messages *say*: every boss arrival credits a randomly generated beastmaster who does not exist (#102); a player-summoned boss is announced twice, out of order, under two different names (#103); `was summoned from the ring` describes a monster leaving (#104); the boss warning is the only ring line with no full stop (#105); and a three-monster fight's summary drops whichever contestant's outcome the branch does not enumerate (#106). #107 adds owner-designed feed dividers — `you joined here`, `connection lost`, `reconnected` — driven by the history/live boundary and the subscription's own error and handshake signals rather than a timestamp gap, which would misfire constantly given the 20–35 min boss spawn window. #108 is its prerequisite: the client discards `heartbeat` events and watches nothing for their absence, so a silently dead connection is never noticed. Screenshots are in `assets/ui-bugs-2026-09/`.
