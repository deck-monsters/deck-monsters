...
`clearRing()` disposed player monsters' healing timers — #156), a Delayed Hit answering a
...
state (#156). The same session confirmed two reports earlier passes had treated as narration
problems: a Delayed Hit firing after an unrelated Heal because two armed copies checked in
nesting order and one counter-attack landed after the other's check (#157), and the Console's
"waiting for your answer" banner rendering over a prompt whose choices were on screen, when
#142 built it for a prompt the player could not see (#158). The same battle showed the Ring
...
(#159). Following is now disabled only after a real scroll gesture. Root causes and tests in
`10b-bugs-fixed.md`.
...
- The Ring now considers the reader pinned within 72px of the bottom, preventing fractional
  ...
  **Superseded by #159**: the 72px tolerance also disabled Virtuoso's snap-back for a follow
