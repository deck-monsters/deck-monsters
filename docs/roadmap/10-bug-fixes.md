...
turn-old blow after an unrelated Heal (#155), the waiting banner covering the prompt
choices it was explaining (#156), and the Ring feed silently falling behind the newest
...
(#3, #51–#58, #59–#73, #74–#85, #86–#97, #98–#111, #112–#134, #143, #146, #147, #151–#157).

## Recently resolved

### September 19 post-battle regressions — FIXED (#154–#157)
...
#142 built it for a prompt the player could not see (#156). The same battle showed the Ring
feed sitting a few lines above the newest narration in auto-scroll mode: the pane treated
every "not at bottom" from Virtuoso as the reader scrolling up, and #148's 72px tolerance —
meant to make that rarer — had also switched off Virtuoso's own snap-back for a follow
scroll that ended short, so the lag was never corrected and `↓ Latest` never appeared
(#157). Following is now disabled only after a real scroll gesture. Root causes and tests in
`10b-bugs-fixed.md`.
...
rather than an impossible pre-clamp value. Follow-up review also closed two recovery-clock
loopholes: combat/full-health time is reset rather than banked, and an instant-revival item
starts healing at the instant it revives the monster while cancelling the old natural timer.
...
- The Ring now considers the reader pinned within 72px of the bottom, preventing fractional
  ...
  **Superseded by #157**: the 72px tolerance also disabled Virtuoso's snap-back for a follow
  scroll that ended short, so the feed lagged silently during bursts. The tolerance is 8px
  again and follow mode is disabled only after a real scroll gesture.
