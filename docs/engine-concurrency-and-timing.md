...
- **Client** (`apps/web/src/hooks/useRingFeed.tsx`, consumed by `Terminal` → `RingPane` / `ConsolePane`): one `ringFeed` subscription per Terminal/room owns the shared reconnect cursor. The cursor updates on every event **except** `handshake` and `heartbeat` (transport frames, not stream positions); `onError` copies it into the subscription input so reconnect carries a cursor. Handshake / protocol reload runs once in this hook, then events fan out to pane listeners. Each pane still fetches its own DB-backed history (`consoleHistory` / `ringHistory`) once on mount, merges under a `historyApplied` guard, and may `seedCursor` when the shared cursor is still empty — history covers page loads, not reconnects.
