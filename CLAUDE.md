...
**Open high-priority bug: fight rewards (coins and xp) may never be credited** — a player
with 2 wins and 9 losses and no purchases saw 0 coins (>=28 expected). `Ring` records
wins/losses directly, but coins and xp are credited only inside `Game.handleWinner`/
`handleLoser`/etc., which are gated behind `createRoomScopedEventGuard`; a false negative
there loses the reward silently. Leading hypothesis and the decisive next diagnostic
(check whether xp also failed to accrue) are in `docs/roadmap/10-bug-fixes.md` item J.
This supersedes the incorrect closure recorded as #145. Note no existing test runs a real
`ring.fight()` and asserts balances — they all emit the handler events directly, bypassing
the guard.

Three further open non-blocking items from the September 2026 live-play pass are tracked in `docs/roadmap/10-bug-fixes.md` (email-defaulted profile names, same-tick roll pairs, fight-open burst). The September 16 2026 mobile UI pass (#98–#111) and the September 17 post-merge passes (#112–#134) are fully resolved. Previously listed items are fixed — see `docs/roadmap/10b-bugs-fixed.md` for root causes, including DMG/CARDS content differentiation (#3), fight log not updating (#15), console history missing on reconnect (#16/#17), card shop room-scoping (#26), batch-equip UX (#19), Discord free-text prompts / serialization (#59/#60), workshop↔console same-user guard (#61), dual web `ringFeed` (#63), harness lane alignment (#73), ConnectorAdapter prompt cancellation (#78), the 2026-08-03 audit fixes (#51–#58, #74–#85), Fastify tRPC batch `maxParamLength` 404s (#86), the preset casing / command-parsing fixes (#87–#88), nested card-play pacing (#89), the missing fight-winner banner (#90), the equip deck-accounting / starting-deck refill bugs (#91–#92), the jump-button / command-echo / email-display / beginner-level fixes (#93–#96), the turn-banner collapse (#97), and the September 16 2026 mobile UI pass — feed text clipped by padding on the Virtuoso scroller (#98), engine `*bold*`/`_italic_` printed literally (#99), `in 1 rounds` (#100), the unrenderable dice turn-banner (#101), bosses credited to a generated beastmaster (#102), the double boss-summon announcement (#103), the ring-exit wording and the player/boss voice reframe (#104), a missing full stop (#105), a dropped contestant in three-way summaries (#106), feed dividers for joins and disconnects (#107), the discarded heartbeat (#108), the fight log returning other players' private events (#109), console fight highlights (#110), item help that taught the opposite of the mid-fight rule (#111), leaderboards showing raw email addresses (#112), the workshop collapsing to a sliver with no monsters (#113), clipped leaderboard columns (#114), a "Send to ring" button that confirmed then failed (#115), the workshop crushing its own monster row instead of scrolling (#116), fight summaries and the room member list leaking the same raw emails as the leaderboards (#117/#118), the fight log and leaderboard rendering a bare frame when empty (#119), the card-slot meter's label sitting unreadably on its own fill (#120), the inventory and slot grids disagreeing on card size at phone width (#121), nothing indicating a second monster existed in the phone carousel (#122), item use controls hanging outside their panel at phone width (#123), boss turn banners crediting an invented beastmaster because only arrival/departure were patched (#124), fight-log list markers clipped on iOS (#125, unconfirmed on device), handbook quick links doing nothing when the console was not in a slot (#126), a stuck "reconnecting" banner on a healthy connection (#127), a stranded "connection lost" divider with the feed running past it (#128), a console that fought the reader's scroll because it drove follow-output imperatively (#129), delayed hits landing with nothing tying them to the card that armed them (#130), the delayed hit's payoff naming no cause when it fired (#131), an unmemoised hook result re-pinning the console to the bottom on every append (#132), a dead console swallowing the next handbook quick link (#133), direct named item transfers selecting without moving the item (#134), the shop's off-by-one Items/Cards/Back Room menus silently routing to the wrong destination (#143), cards on sale being invisible and unbuyable from the Workshop (#144), and the Workshop wallet being invisible on load and up to 30s stale after a fight (#145).
